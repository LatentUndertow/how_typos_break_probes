...
class InjecAgentDataset(Dataset):
    ...
    DATASET_META = DatasetMeta(default_params={'attack_types': ['dh', 'ds'], 'setting': 'base'}, has_spans=True, category='indirect_pi', max_samples=1054)
    ...
    def __init__(
        ...
        if not self.injecagent_root.exists():
            raise FileNotFoundError(
                f"InjecAgent root directory not found: {self.injecagent_root}\n"
            )
        ...
        if not self.data_dir.exists():
            raise FileNotFoundError(
                f"InjecAgent data directory not found: {self.data_dir}\n"
            )
    ...
    def get_injection_span(
        ...
        """Find attacker instruction span by exact substring match only."""
        attack_text = labels.get("attacker_instruction", "")
        if not attack_text:
            return None

        start = input_text.find(attack_text)
        if start < 0:
            return None

        # Ambiguous multiple matches are considered invalid for token labels.
        if input_text.find(attack_text, start + 1) != -1:
            return None

        return (start, start + len(attack_text))
